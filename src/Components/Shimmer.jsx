import React, { useEffect, useState } from "react";
import { funfacts } from "../Constants/Constants";

const Shimmer = () => {
  const [randomnumber, setrandomnumber] = useState(0);
  useEffect(() => {
    setrandomnumber(Math.floor(Math.random() * funfacts.length));
  }, []);
  console.log(randomnumber);
  return (
    <div className="">
      <div className="w-[30%] h-[20%] m-auto">
        <img
          src="https://camo.githubusercontent."
          alt=""
        />
      </div>
      <div className="w-[30%] h-[20%] m-auto text-center">
        <p className="text-2xl font-bold">Fun Fact:{funfacts[randomnumber]}</p>
      </div>
    </div>
  );
};

export default Shimmer;
